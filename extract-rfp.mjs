...
export async function extractRecord(recordPath) {
  ...
  const merged = mergeExtractedFields(opportunity, extractOpportunityFields(text));
...
async function main() {
  ...
  if (!input || process.argv.includes('--help')) {
    console.log('Usage: node extract-rfp.mjs <data/opportunities/id/opportunity.yml>');
    process.exit(input ? 0 : 1);
  }
  console.log(JSON.stringify(await extractRecord(input), null, 2));
}
